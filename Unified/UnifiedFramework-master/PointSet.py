...
class PointSet(object):
    def __init__(self, P, W=None, ellipsoid_max_iters=10, problem_type=None, use_svd=False, compute_U=True):
        ...
        # The line bellow was modified due to an error caused by the line.
        self.cost_func = lambda x: np.linalg.norm(np.multiply(self.W, np.dot(self.P, x)),ord=1).sum()
    ...
    def computeU(self):
        ...
        if self.use_svd:
            ...
            B = np.diag(self.D).__mul__(self.V)
            ans = np.linalg.inv(B)
            self.U = np.dot(self.P,ans)
